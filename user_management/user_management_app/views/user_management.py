...
from user_management_app.constants.error import INVALID_REQUEST, USER_DOESNT_EXIST, USER_OR_PERMISSION_DOESNT_EXIST
from user_management_app.models import User, Permission, UserPermissions
from user_management_app.serializers.user import UserPermissionSerializer
...
class UserPermissionView(GenericAPIView):
    ...
    def get(self, request, user_id=None, permission_id=None):
        if user_id:
            ...
            if user:
                user_permissions = UserPermissions.objects.filter(user=user).all()
